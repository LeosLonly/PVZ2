...
import edu.bistu.pvz.bean.base.Plant;
import edu.bistu.pvz.bean.plant.PlantNut;
import edu.bistu.pvz.bean.plant.PlantPease;
import edu.bistu.pvz.bean.plant.PlantPeases;
import edu.bistu.pvz.bean.plant.PlantSun;
import edu.bistu.pvz.bean.product.Sun;
import edu.bistu.pvz.bean.zombies.ZombiesPrimary;
import edu.bistu.pvz.layer.FightLayer;
...
public class GameController {
	...
	public void startGame(CCTMXTiledMap gameMap, List<ShowPlant> plants) {
		...
		updatePlantSelect();
		//生成太阳
		CCScheduler.sharedScheduler().schedule("productSun", this, 20, false);
		progress();
		//new SunProductor();
	}
	...
	public void productSun(float f) {
		Random random = new Random();
		int x = random.nextInt((int) winSize.width);
		int y = (int) winSize.height;
		new Sun(gameMap.getParent(), Sun.TYPE_BIG, ccp(x, y), ccp(x,
				y-200));
	}
	...
	private void loadTower() {
		for (int row = 0; row < 5; row++) {
			...
			for (HashMap<String, String> item : towerGroup.objects) {
				towers[row][coloum] = ccp(Float.parseFloat(item.get("x")), Float.parseFloat(item.get("y")) - 15);
				coloum++;
			}
		}
	...
	private void loadRoad() {
		...
		for (HashMap<String, String> item : roadGroup.objects) {
			roads.add(ccp(Float.parseFloat(item.get("x")), Float.parseFloat(item.get("y"))));
		}
