...
import edu.bistu.pvz.bean.ShowPlant;
import edu.bistu.pvz.bean.base.Zombies;
import edu.bistu.pvz.bean.zombies.ZombiesPrimary;
import edu.bistu.pvz.engine.GameController;
import edu.bistu.pvz.layer.manager.BaseLayer;
import edu.bistu.pvz.layer.manager.LayerManager;
import edu.bistu.pvz.util.CommonUtil;
...
//		}
    ...
    public void endGame() {
//        FightLayer.this.removeSelf();
//        new FightLayer();
//        CCSprite endGame = CCSprite.sprite("image/fight/ZombiesWon.jpg");
//        endGame.setAnchorPoint(0, 0);
//        endGame.setPosition(gameMap.getContentSize().width / 2, gameMap.getContentSize().height / 2);
//        gameMap.addChild(endGame);
...
//        this.runAction(sequence);
        //restart();
        FightLayer.getContext().finish();
    }

    public void restart() {
        FightLayer.chosePlants.clear();
        FightLayer.getContext().finish();
        plantChoosePool.removeSelf();
        plantChosePool.removeSelf();
        plantChosePool.removeAllChildren(true);
        plantChoosePool.removeAllChildren(true);
            ...
            if (CGRect.containsPoint(plantChosePool.getBoundingBox(), convertTouchToNodeSpace(event))) {
                ...
                if (isClick(event, plantChoosePool.getChildByTag(TAG_START_GAME))) {
                    ...
                    for (int i = 0; i < plantNum; i++) {
                        ...
                        if (isClick(event, showPlant.getPlant()) && chosePlants.size() <= 4) {
                            // 用户选择了一个植物
    ...
    public CCSprite getPlantChosePool() {
        return plantChosePool;
    }

    public CCSprite getPlantChoosePool() {
        return plantChoosePool;
    }

    public static CopyOnWriteArrayList<Integer> getChosePlants() {
        return chosePlants;
    }
}
